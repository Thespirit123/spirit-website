...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        const { companyApiId, cableNumber, planId, amount } = await req.json();

        if (!companyApiId || !cableNumber || !planId || !amount) {
        ...
        const ncWalletResponse = await fetch(
            "https://ncwallet.africa/api/v1/cable",
            {
                method: "POST",
                headers: {
        ...
        if (purchaseResult.status !== "success") {
            ...
            throw new Error(
                providerMessage || "The cable provider failed to process the request."
            );
        }
        ...
        batch.set(transactionRef, {
            ...
            description: `Cable TV subscription for ${cableNumber}`,
            paymentMethod: "wallet",
            reference: purchaseResult.data?.reference || transactionId,
            status: "success",
        ...
        return NextResponse.json({
            status: "success",
            message: `Successfully subscribed ${cableNumber}.`,
            transactionId,
        });
    } catch (error) {
        const message =
